			   ...
			   u8 command, int size, union i2c_smbus_data *data)
{
	 ...
	 * switched to P2WI (which means the bootloader has to switch the slave
...
static int p2wi_probe(struct platform_device *pdev)
{
	...
	/*
	 * Authorize no child definition to be able to use an i2c-dev from
	 * userpace.
	 * In this case the slave_addr is set to -1 and won't be check when
